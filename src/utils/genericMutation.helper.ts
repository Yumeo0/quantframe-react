...
export const createGenericMutation = <TData, TVariables>(
	...
	return useMutation({
		...
		onSuccess: (data: TData, variables: TVariables) => {
			const refetchStatusString = ["create_stock_item"];
			hooks.refetchQueries(refetchStatusString.includes(config.successKey));
			const isMultiple = config.isMultiple
				? config.isMultiple(variables)
				: false;
			notifications.show({
				title: useTranslateCommon(
					`notifications.${config.successKey}.success.title${isMultiple ? "_multiple" : ""}`,
				),
		...
		onError: (e: any, variables: TVariables) => {
			...
			notifications.show({
				title: useTranslateCommon(
					`notifications.${config.errorKey}.error.title${isMultiple ? "_multiple" : ""}`,
				),
				message: useTranslateCommon(
					`notifications.${config.errorKey}.error.message${isMultiple ? "_multiple" : ""}`,
				),
				color: "red.7",
			});
		},
