...
const CompareValues = (
	value: any,
	filterValues: OperatorConditionMap,
): boolean => {
	...
	const operators = Object.keys(filterValues).filter(
		(operator) => !excludeProperties.includes(operator),
	);
	return operators.every((operator) => {
		...
		if (!filterValues.isCaseSensitive) {
			...
			if (Array.isArray(fValue))
				fValue = fValue.map((v) => v.toString().toLowerCase());
...
const ProcessFilterConditions = <T>(
	item: T,
	...
	return Object.entries(filterValue).every(([filterName, filterValue]) => {
		...
		if (filterValue.combineFields)
			propertyValue = CombineFields(
				item,
				filterValue.combineFields,
				filterValue.combineWith || "",
			);
