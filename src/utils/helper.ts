...
import type { TauriTypes, WFMarketTypes } from "$types";
...
export const groupBy = <T, K extends keyof T>(
	...
	return array.reduce(
		(acc, cur) => {
			...
			// initialize the group array if it doesn't exist, then push
			if (!acc[groupByKey]) {
				acc[groupByKey] = [];
			}
...
export const GetSubTypeDisplay = (
	subType: TauriTypes.SubType | WFMarketTypes.Order | undefined,
) => {
...
export const CreateTradeMessage = (
	prefix: string,
