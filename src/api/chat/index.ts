...
export class ChatModule {
	...
	async refresh(): Promise<WFMarketTypes.ChatData[]> {
		return await this.client.sendInvoke<WFMarketTypes.ChatData[]>(
			"chat_refresh",
		);
	}

	async getPagination(
		query: WFMarketTypes.WfmChatDataControllerGetListParams,
	): Promise<WFMarketTypes.WfmChatDataControllerGetListData> {
		return await this.client.sendInvoke<WFMarketTypes.WfmChatDataControllerGetListData>(
			"get_chat_pagination",
			{ query },
		);
	}
	...
	async getChatMessages(id: string): Promise<WFMarketTypes.ChatMessage[]> {
		return await this.client.sendInvoke<WFMarketTypes.ChatMessage[]>(
			"chat_get_messages_by_id",
			{ id },
		);
	}
