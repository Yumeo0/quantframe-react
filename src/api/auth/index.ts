...
export class AuthModule {
	...
	async login(
		email: string,
		password: string,
	): Promise<TauriTypes.User | undefined> {
		return await this.client.sendInvoke<TauriTypes.User>("auth_login", {
			email,
			password,
		});
	}
	...
	async hasPermission(flag: TauriTypes.PermissionsFlags): Promise<boolean> {
		if (this.permissionsCache[flag] !== undefined)
			return this.permissionsCache[flag];

		const result = await this.client.sendInvoke<boolean>(
			"auth_has_permission",
			{ flag },
		);
