...
export class AppModule {
	...
	get_settings() {
		return useQuery({
			...
			queryFn: () =>
				this.client.sendInvoke<TauriTypes.Settings>("app_get_settings"),
			retry: false,
	...
	get_app_info() {
		return useQuery({
			...
			queryFn: () =>
				this.client.sendInvoke<TauriTypes.AppInfo>("app_get_app_info"),
			retry: false,
			enabled: false, // Disable automatic fetching
		});
	}
	updateSettings(settings: TauriTypes.Settings): Promise<TauriTypes.Settings> {
		return this.client.sendInvoke<TauriTypes.Settings>("app_update_settings", {
			settings,
		});
	}
	exit() {
	...
	notify_reset(id: string): Promise<TauriTypes.NotificationSetting> {
		return this.client.sendInvoke<TauriTypes.NotificationSetting>(
			"app_notify_reset",
			{ id },
		);
	}
