...
export class TauriClient {
	...
	constructor() {
		...
		this._logging = localStorage.getItem("tauri_logs")
			? JSON.parse(localStorage.getItem("tauri_logs") ?? "[]")
			: ["*"];
	}
	...
	private logInvoke(command: string, data?: any, response?: any, error?: any) {
		...
		if (this._logging.includes("*"))
			this._loggingCount["*"] = (this._loggingCount["*"] || 0) + 1;

		if (
			!this._logging.includes(command) &&
			!this._logging.includes("*") &&
			!error
		)
			return;
		...
		const successStyle =
			"color: #16a34a; font-weight: bold; background: #f0fdf4; padding: 2px 4px; border-radius: 3px;";
		...
		if (response !== undefined)
			console.log(`%cResponse:`, responseStyle, response);
	...
	getLogging(search: string): { command: string; count: number }[] {
		...
		if (search)
			filteredLogs = this._logging.filter((log) =>
	...
	objectToParameters(obj: any): Array<string> {
		...
		for (let index = 0; index < entries.length; index++) {
			...
			if (Array.isArray(element[1])) {
				...
				searchParams.push(
					array.map((item) => `${element[0]}=${item}`).join("&"),
				);
...
const OnTauriEvent = <T>(event: string, callback: (data: T) => void) =>
	window.api.events.OnEvent(event, callback);
const OnTauriDataEvent = <T>(
	event: string,
	callback: (data: { operation: TauriTypes.EventOperations; data: T }) => void,
) => window.api.events.OnEvent(event, callback);

const OffTauriEvent = <T>(event: string, callback: (data: T) => void) =>
	window.api.events.OffEvent(event, callback);
const OffTauriDataEvent = <T>(
	event: string,
	callback: (data: { operation: TauriTypes.EventOperations; data: T }) => void,
) => window.api.events.OffEvent(event, callback);

const SendTauriEvent = async (event: string, data?: any) =>
	window.api.events.FireEvent(event, data);
const SendTauriDataEvent = async (
	event: string,
	operation: TauriTypes.EventOperations,
	data: any,
) => window.api.events.FireEvent(event, { operation, data });
const WFMThumbnail = (thumb: string) =>
...
const HasPermission = async (
	flag: TauriTypes.PermissionsFlags,
): Promise<boolean> => {
