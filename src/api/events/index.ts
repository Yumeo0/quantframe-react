...
export class EventModule {
	...
	private logEvent(
		...
		this.client._loggingCount[event] =
			(this.client._loggingCount[event] || 0) + 1;
		if (this.client._logging.includes("*"))
			this.client._loggingCount["*"] =
				(this.client._loggingCount["*"] || 0) + 1;

		if (
			!this.client._logging.includes(event) &&
			!this.client._logging.includes("*")
		)
			return;
		...
		if (operation)
			console.log(`%cOperation: %c${operation}`, dataStyle, "color: #fff");
	...
	private async Initializer() {
		...
		listen(
			"message_update",
			(eventIn: {
				payload: { event: string; operation: string; data: any };
			}) => {
