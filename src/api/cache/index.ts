...
export class CacheModule {
	...
	async getTradableItems(): Promise<TauriTypes.CacheTradableItem[]> {
		...
		const items = await this.client.sendInvoke<TauriTypes.CacheTradableItem[]>(
			"cache_get_tradable_items",
		);
		this._cache.set(CacheType.TradableItems, items);
		return items;
	}
	async getTradableItemById(
		id: string,
	): Promise<TauriTypes.CacheTradableItem | undefined> {
	...
	getThemePresets() {
		return useQuery({
			...
			queryFn: () =>
				this.client.sendInvoke<TauriTypes.CacheTheme[]>(
					"cache_get_theme_presets",
				),
			retry: false,
		});
	}
	createTheme(name: string, author: string, properties: any): Promise<void> {
		return this.client.sendInvoke<void>("cache_create_theme", {
			name,
			author,
			properties,
		});
	...
	async getRivenAttributes(): Promise<TauriTypes.CacheRivenAttribute[]> {
		...
		const items = await this.client.sendInvoke<
			TauriTypes.CacheRivenAttribute[]
		>("cache_get_riven_attributes");
	...
	async getRivenWeapons(): Promise<TauriTypes.CacheRivenWeapon[]> {
		...
		let items = await this.client.sendInvoke<TauriTypes.CacheRivenWeapon[]>(
			"cache_get_riven_weapons",
		);
		items = items.filter((i) => !i.is_variant);
		this._cache.set(CacheType.RivenWeapons, items);
		return items;
	}
	async getWeaponByUrl(
		id: string,
	): Promise<TauriTypes.CacheRivenWeapon | undefined> {
