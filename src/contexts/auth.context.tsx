import api, { OffTauriDataEvent, OnTauriDataEvent } from "@api/index";
...
export function AuthContextProvider({ children }: TauriContextProviderProps) {
	...
	useEffect(() => {
		OnTauriDataEvent<TauriTypes.User>(
			TauriTypes.Events.UpdateUser,
			({ data, operation }) => handleUpdateUser(operation, data),
		);
		return () => {
			OffTauriDataEvent<TauriTypes.User>(
				TauriTypes.Events.UpdateUser,
				({ data, operation }) => handleUpdateUser(operation, data),
			);
		};
