...
const validateTheme = (theme: Record<string, any>) => {
	...
	for (const key in properties.colors)
		if (theme.colors[key] === undefined)
			theme.colors[key] = properties.colors[key];
	for (const key in properties.other) {
		const typedKey = key as keyof typeof properties.other;
		if (theme.other === undefined) theme.other = {};
		if (theme.other[typedKey] === undefined)
			theme.other[typedKey] = properties.other[typedKey];
	}
	return theme;
};
		...
		(themeData: string) => {
			try {
				...
				notifications.show({
					title: "Import Error",
					message:
						"Failed to import theme. Please ensure the JSON format is correct.",
					color: "red",
				});
