...
export type LiveScraperContextProps = {
	is_running: boolean;
	message?: { i18nKey: string; values: Record<string, unknown> };
};
...
export function LiveScraperContextProvider({
	children,
}: LiveScraperContextProviderProps) {
	const [is_running, setIsRunning] = useState(false);
	const [message, setMessage] = useState<
		{ i18nKey: string; values: Record<string, unknown> } | undefined
	>(undefined);
	...
	useEffect(() => {
		...
		OnTauriEvent<boolean>(
			TauriTypes.Events.UpdateLiveScraperRunningState,
			(data) => setIsRunning(data),
		);
		OnTauriEvent<
			{ i18nKey: string; values: Record<string, unknown> } | undefined
		>(TauriTypes.Events.OnLiveScraperMessage, (data) => setMessage(data));
		return () => {
			OffTauriEvent<boolean>(
				TauriTypes.Events.UpdateLiveScraperRunningState,
				(data) => setIsRunning(data),
			);
			OffTauriEvent<
	...
	return (
		<LiveScraperContext.Provider value={{ is_running, message }}>
			{children}
