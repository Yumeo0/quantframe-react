...
// Add a clear type for the starting-up state instead of using '{}' as a type
type StartingUpState = {
	i18n_key: string;
...
export function AppContextProvider({ children }: AppContextProviderProps) {
	...
	const [startingUp, setStartingUp] = useState<StartingUpState>({
	...
	const handleOnNotify = (data: {
		...
		notifications.show({
			title: useTranslateCommon(
				`notifications.${data.i18n_key}.${data.type}.title`,
				data.values,
			),
			color: data.color,
			message: (
				<TextTranslate
					i18nKey={useTranslateCommon(
						`notifications.${data.i18n_key}.${data.type}.message`,
						undefined,
						true,
					)}
					values={data.values}
	...
	const checkForUpdates = async (
		...
		const update = await check({
			headers: { IsPreRelease: info.is_pre_release ? "true" : "false" },
		});
		if (notifyIfNone && !update)
			notifications.show({
				...
				message: useTranslateCommon(
					"notifications.no_updates_available.message",
				),
			});
		if (!update) return;
		modals.open({
			title: useTranslateComponent("modals.update_available.title", {
	...
	}, [app_info, checkForTosUpdates, checkForUpdates]);
	...
	return (
		...
		>
			<SplashScreen
				opened={loading}
				text={useTranslateContexts(
					`app.${startingUp.i18n_key}`,
					startingUp.values,
				)}
			/>
			{!loading && (
