...
import type { QuantframeApiTypes, TauriTypes, WFMarketTypes } from "$types";
...
export function ItemName({ color, size, hideQuantity, value }: ItemNameProps) {
	...
	const GetName = () => {
		...
		if ("wfm_id" in value && !fullName)
			fullName =
				data.data?.find((i) => i.wfm_id === value.wfm_id)?.name ||
				value.wfm_id ||
				"Unknown Item";
		...
		if (
			"properties" in value &&
			value.properties &&
			"mod_name" in value.properties
		)
			fullName += ` ${value.properties.mod_name}`;
	...
	const GetQuantity = (): string | number => {
		if (!value) return "";
		let quantity = 0;
		if ("quantity" in value && value.quantity && !hideQuantity)
	...
	return (
		<Group align="center">
			<TextTranslate
				...
				values={{
					name: GetName(),
					sub_type: GetSubTypeDisplay(GetSubType()),
