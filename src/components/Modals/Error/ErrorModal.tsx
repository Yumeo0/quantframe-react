...
export function ErrorModal({ error, app_info }: ErrorModalProps) {
	const useTranslateModal = (
		...
		raw?: boolean,
	) => useTranslateModals(`app_error.${key}`, ctx, raw);
		...
		<Box>
			...
			>
				...
				<TextTranslate
					size="md"
					color="white"
					i18nKey={useTranslateModal("cause", undefined, true)}
					values={{ cause: error?.cause || "Unknown" }}
				/>
				<TextTranslate
					size="md"
					color="white"
				...
				<TextTranslate
					color="white"
					i18nKey={useTranslateModal("location", undefined, true)}
					values={{ location: error?.location || "Unknown" }}
				/>
