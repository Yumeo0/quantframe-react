...
export function AnalyticsTab({ value }: AnalyticsTabProps) {
	...
	const useTranslateTab = (
		...
		useTranslateModals(
			`stock_riven_details.tabs.analytics.${key}`,
			{ ...context },
			i18Key,
		);
	return (
		<Grid>
			<Grid.Col span={6}>
				<Paper>
					<Group p={10} justify="space-between">
						...
						<Group>
							<ColorInfo
								...
								text={useTranslateTab("last_transactions.info_box.purchase", {
									count: value.last_transactions.length || 0,
								})}
							/>
							<ColorInfo
								infoProps={{
								...
								text={useTranslateTab("last_transactions.info_box.sale", {
									count: value.last_transactions.length || 0,
								})}
							/>
						</Group>
					...
					<Divider />
					<ScrollArea p={10}>
						{value.last_transactions.map((transaction) => (
							<TransactionListItem
								key={`${transaction.id}`}
					...
					segments={[
						{
							...
							count: value.expenses,
							part: value.average_expense,
