...
import { Center, Grid, Group, Text, TextInput, Title } from "@mantine/core";
...
export function WFMTab({ value }: WFMTabProps) {
	...
	const useTranslateTab = (
		...
		useTranslateModals(
			`stock_riven_details.tabs.wfm.${key}`,
			{ ...context },
			i18Key,
		);
	...
	if (!value || !value.auction_info)
		return <Text ta="center">{useTranslateTab("no_auction_info")}</Text>;
	return (
		<Grid>
			<Grid.Col span={4}>
				<Group grow>
					<TextInput
						...
						value={dayjs(value.auction_info.created).format(
							"DD/MM/YYYY HH:mm:ss",
						)}
						readOnly
					/>
					<TextInput
						...
						value={dayjs(value.auction_info.updated).format(
							"DD/MM/YYYY HH:mm:ss",
						)}
						readOnly
					/>
