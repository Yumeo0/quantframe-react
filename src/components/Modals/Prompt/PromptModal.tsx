...
export function PromptModal({
	...
	id,
	innerProps,
}: ContextModalProps<PromptModalProps>) {
	const { height, confirmLabel, cancelLabel, fields, onConfirm, onCancel } =
		innerProps;
	...
	for (let index = 0; index < fields.length; index++) {
		...
		switch (field.type) {
			...
			case "slider":
				...
				break;
			case "select":
				formValues[field.name] = field.options
			...
			<Container size="auto" h={height} pt={25}>
				{fields.map((field) => {
					switch (field.type) {
						case "text":
							return (
								<Group grow key={`${field.name}${field.value}`}>
									<TextInput
