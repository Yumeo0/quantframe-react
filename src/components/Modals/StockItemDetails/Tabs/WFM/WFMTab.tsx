...
export function WFMTab({ value }: WFMTabProps) {
	...
	const useTranslateTab = (
		...
		useTranslateModals(
			`stock_item_details.tabs.wfm.${key}`,
			{ ...context },
			i18Key,
		);
	...
	if (!value)
		return <Text ta="center">{useTranslateTab("no_order_info")}</Text>;
	if (!value.order_info)
		return (
			<Button
				mt={"md"}
			...
			<Grid.Col span={6}>
				<Group grow>
					<TextInput
						...
						value={dayjs(value.order_info.createdAt).format(
							"DD/MM/YYYY HH:mm:ss",
						)}
						readOnly
					/>
					<TextInput
						...
						value={dayjs(value.order_info.updatedAt).format(
							"DD/MM/YYYY HH:mm:ss",
						)}
						readOnly
					/>
