...
export function OverviewTab({ value }: OverviewTabProps) {
	...
	const useTranslateTab = (
		...
		useTranslateModals(
			`stock_item_details.tabs.overview.${key}`,
			{ ...context },
			i18Key,
		);
			...
			<Grid.Col span={6}>
				<Group grow>
					<TextInput
						...
						value={dayjs(value.stock.created_at).format("DD/MM/YYYY HH:mm:ss")}
						readOnly
					/>
					<TextInput
						label={useTranslateFields("updated_at")}
						value={dayjs(value.stock.updated_at).format("DD/MM/YYYY HH:mm:ss")}
						readOnly
					/>
				</Group>
				<Group grow>
					...
					<TextInput
						...
						value={value.stock.minimum_price || "N/A"}
						readOnly
					/>
					<TextInput
						label={useTranslateFields("owned")}
						value={value.stock.owned || "N/A"}
						readOnly
					/>
				</Group>
				<Group grow>
					<TextInput
						...
						value={value.stock.bought}
						readOnly
					/>
					<TextInput
						label={useTranslateFields("list_price")}
						value={value.stock.list_price || "N/A"}
						readOnly
					/>
				</Group>
				<Divider mt={"md"} />
				<Group grow>
					<TextInput
						...
						value={value.potential_profit || "N/A"}
						readOnly
					/>
					<TextInput
						label={useTranslateFields("highest_price")}
						value={value.order_info?.properties?.highest_price || "N/A"}
						readOnly
					/>
					<TextInput
						label={useTranslateFields("lowest_price")}
						value={value.order_info?.properties?.lowest_price || "N/A"}
						readOnly
					/>
				</Group>
				{value.item_info?.components &&
					value.item_info.components.length > 0 && (
						<>
							<Divider mt={"md"} />
							<ItemComponents components={value.item_info.components} />
						</>
					)}
				<Divider mt={"md"} />
				{value.item_info && (
