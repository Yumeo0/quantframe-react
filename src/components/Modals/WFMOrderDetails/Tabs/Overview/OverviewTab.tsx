...
export function OverviewTab({ value }: OverviewTabProps) {
	// Translate general
	const useTranslateTab = (
		key: string,
		context?: { [key: string]: any },
		i18Key?: boolean,
	) =>
		useTranslateModals(
			`wfm_order_details.tabs.overview.${key}`,
			{ ...context },
			i18Key,
		);
	...
	return (
		<Grid>
			<Grid.Col span={6}>
				<Group grow>
					<TextInput
						label={useTranslateFields("created_at")}
						value={dayjs(order_info?.createdAt).format("DD/MM/YYYY HH:mm:ss")}
						readOnly
					/>
					<TextInput
						label={useTranslateFields("updated_at")}
						value={dayjs(order_info?.updatedAt).format("DD/MM/YYYY HH:mm:ss")}
						readOnly
					/>
				</Group>
				<Group grow>
					...
					/>
					<TextInput
						label={useTranslateFields("operations")}
						value={order_info?.properties?.operation.join(", ") || "N/A"}
						readOnly
					/>
				</Group>
				<Group grow>
					<TextInput
						label={useTranslateFields("platinum")}
						value={order_info?.platinum || "N/A"}
						readOnly
					/>
					<TextInput
						label={useTranslateFields("quantity")}
						value={order_info?.quantity || "N/A"}
						readOnly
					/>
					<TextInput
						label={useTranslateFields("per_trade")}
						value={order_info?.perTrade || "N/A"}
						readOnly
					/>
				</Group>
				<Divider mt={"md"} />
				<Group grow>
					<TextInput
						label={useTranslateFields("closed_avg")}
						value={order_info?.properties?.closed_avg || "N/A"}
						readOnly
					/>
					<TextInput
						label={useTranslateFields("profit")}
						value={order_info?.properties?.profit || "N/A"}
						readOnly
					/>
					<TextInput
						label={useTranslateFields("highest_price")}
						value={order_info?.properties?.highest_price || "N/A"}
						readOnly
					/>
					<TextInput
						label={useTranslateFields("lowest_price")}
						value={order_info?.properties?.lowest_price || "N/A"}
						readOnly
					/>
				</Group>
					...
					order_info?.properties?.price_history
						...
						.map((price) => (
							<PriceHistoryListItem
								key={`${price.created_at}${price.price}`}
								history={price}
							/>
						))}
			</Grid.Col>
