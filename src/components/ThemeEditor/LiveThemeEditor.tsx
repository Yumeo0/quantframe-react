...
function ChartColorPaletteEditor({
	colorName,
	colors,
	onColorChange,
	baseTranslation,
}: ChartColorPaletteEditorProps) {
	return (
		<Box mb="md">
			<Text size="md" fw={500}>
				{colorName}
...
export function LiveThemeEditor({ onNewTheme }: LiveThemeEditorProps) {
	...
	const export_form = useForm({
		...
		validate: {
			name: (value: string) =>
				value.length < 3 ? useTranslateFormFields("export_name.error") : null,
			author: (value: string) =>
				value.length < 3 ? useTranslateFormFields("export_author.error") : null,
		},
	});

	const exportTheme = useMutation({
		mutationFn: (data: { name: string; author: string; properties: any }) =>
			api.cache.createTheme(data.name, data.author, data.properties),
		onSuccess: () => {
			onNewTheme?.();
			notifications.show({
		...
		onError: (error) => {
			...
			notifications.show({
				...
				message: useTranslateEditor("export_error.message", {
					error: error.message,
				}),
				color: "red",
			});
	...
	return (
		<Accordion variant="separated">
			...
			<Accordion.Item value="color_palettes">
				<Accordion.Control>
					{useTranslateEditor("color_palettes.title")}
				</Accordion.Control>
				<Accordion.Panel>
					<ColorPaletteEditor
						colorName={useTranslateEditor("color_palettes.base_colors")}
						colors={Object.values(colors.dark as string[]).map((c, _) => ({
							value: c,
						}))}
						onColorChange={(index, color) =>
							handleColorChange(`dark`, index, color)
						}
					/>
					<ChartColorPaletteEditor
						...
						onColorChange={(property, color) =>
							handlePropertyChange(`other.stockStatus.${property}`, color)
						}
					/>
					<ChartColorPaletteEditor
						...
						onColorChange={(property, color) =>
							handlePropertyChange(`other.transactionType.${property}`, color)
						}
					/>
					<ChartColorPaletteEditor
						...
						onColorChange={(property, color) =>
							handlePropertyChange(`other.alertType.${property}`, color)
						}
					/>
					<ChartColorPaletteEditor
						...
						onColorChange={(property, color) =>
							handlePropertyChange(`other.userStatus.${property}`, color)
						}
					/>
					<ChartColorPaletteEditor
						...
						onColorChange={(property, color) =>
							handlePropertyChange(`other.itemType.${property}`, color)
						}
					/>
					<Group>
						<ColorInput
							...
							onChangeEnd={(value) =>
								handlePropertyChange("other.logoColor", value)
							}
							w={100}
							withEyeDropper={false}
						/>
						<ColorInput
							...
							onChangeEnd={(value) =>
								handlePropertyChange("other.positiveColor", value)
							}
							w={100}
							withEyeDropper={false}
						/>
						<ColorInput
							...
							onChangeEnd={(value) =>
								handlePropertyChange("other.negativeColor", value)
							}
							w={100}
							withEyeDropper={false}
						/>
						<ColorInput
							...
							value={other.profit}
							onChangeEnd={(value) =>
								handlePropertyChange("other.profit", value)
							}
							w={100}
							withEyeDropper={false}
						/>
			...
			<Accordion.Item value="chart_styles">
				<Accordion.Control>
					{useTranslateEditor("chart_styles.title")}
				</Accordion.Control>
				<Accordion.Panel>
					<Group>
						<ChartColorPaletteEditor
							...
							onColorChange={(property, color) =>
								handlePropertyChange(
									`other.chartStyles.total.${property}`,
									color,
								)
							}
						/>
						<ChartColorPaletteEditor
							...
							onColorChange={(property, color) =>
								handlePropertyChange(
									`other.chartStyles.today.${property}`,
									color,
								)
							}
						/>
						<ChartColorPaletteEditor
							...
							onColorChange={(property, color) =>
								handlePropertyChange(
									`other.chartStyles.lastDays.${property}`,
									color,
								)
							}
						/>
			...
			<Accordion.Item value="sharing">
				<Accordion.Control>
					{useTranslateEditor("sharing.title")}
				</Accordion.Control>
				<Accordion.Panel>
					<Group>
						<Button
								...
								readText().then((text) => {
									if (text) {
										...
										notifications.show({
											...
											message: useTranslateButtons(
												"import_from_clipboard.message",
											),
											color: "green",
										});
									}
						...
						<Button onClick={() => api.cache.openThemeFolder()}>
							{useTranslateButtons("open_themes_folder")}
						</Button>
					</Group>
					<Group mt={"md"}>
						<TextInput
							...
							onChange={(e) =>
								export_form.setFieldValue("name", e.target.value)
							}
						...
						<TextInput
							...
							onChange={(e) =>
								export_form.setFieldValue("author", e.target.value)
							}
							...
							rightSection={
								<ActionWithTooltip
									...
									onClick={() => {
										...
										exportTheme.mutateAsync({
											...export_form.values,
											properties: currentThemeData,
										});
									}}
								/>
							}
