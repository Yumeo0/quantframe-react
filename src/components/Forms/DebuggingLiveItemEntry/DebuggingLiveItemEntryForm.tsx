...
export function DebuggingLiveItemEntryForm({
	disabled,
	boxProps,
	onSubmit,
	initialValues,
}: DebuggingLiveItemEntryFormProps) {
	// Translate general
	...
	// Precompute error translations (call hooks unconditionally)
	const wfmUrlError = useTranslateFormFields("wfm_url.error");
	const priorityError = useTranslateFormFields("priority.error");
	const buyQuantityError = useTranslateFormFields("buy_quantity.error");
	const sellQuantityError = useTranslateFormFields("sell_quantity.error");
	const operationError = useTranslateFormFields("operation.error");
	...
	const form = useForm<TauriTypes.DebuggingLiveItemEntry>({
		...
		validate: {
			wfm_url: (value) => (value.length === 0 ? wfmUrlError : null),
			priority: (value) => (value < 0 ? priorityError : null),
			buy_quantity: (value) => (value < 0 ? buyQuantityError : null),
			sell_quantity: (value) => (value < 0 ? sellQuantityError : null),
			operation: (value) => (value.length === 0 ? operationError : null),
		},
	});
			...
			>
				<Group>
					...
					<NumberInput
						...
						onChange={(value) =>
							form.setFieldValue("stock_id", value ? Number(value) : null)
						}
					...
					<NumberInput
						...
						onChange={(value) =>
							form.setFieldValue("wish_list_id", value ? Number(value) : null)
						}
				...
				<Group gap="md" align="end" className={classes.formGroup}>
					...
					<NumberInput
						...
						onChange={(value) =>
							form.setFieldValue("buy_quantity", Number(value))
						}
					...
					<NumberInput
						...
						onChange={(value) =>
							form.setFieldValue("sell_quantity", Number(value))
						}
