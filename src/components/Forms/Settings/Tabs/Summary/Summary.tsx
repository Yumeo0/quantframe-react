...
export const SummaryPanel = ({ value, onSubmit }: SummaryPanelProps) => {
	...
	const [selectedCategory, setSelectedCategory] = useState<
		TauriTypes.SettingsCategorySummary | undefined
	>(undefined);
	// Translate general
	...
	const useTranslateDataGridBaseColumns = (
		key: string,
		context?: { [key: string]: any },
		i18Key?: boolean,
			...
			{mode === Mode.CreateOrUpdate ? (
				<CreateCategorySummary
					...
					onSubmit={async (data) => {
						setMode(Mode.None);
						if (selectedCategory) {
							const categories = form.values.categories.map((cat) =>
								cat.name === selectedCategory.name ? data : cat,
							);
					...
					<Grid>
							...
							<Group gap="xs" grow>
								<NumberInput
									...
									placeholder={useTranslateFormFields(
										"recent_days.placeholder",
									)}
									value={form.values.recent_days}
									onChange={(event) =>
										form.setFieldValue("recent_days", Number(event))
									}
									error={
										form.errors.recent_days &&
										useTranslateFormFields("recent_days.error")
									}
									rightSection={
										<TooltipIcon
											label={useTranslateFormFields("recent_days.tooltip")}
										/>
									}
									radius="md"
								/>
								<NumberInput
									...
									onChange={(event) =>
										form.setFieldValue("recent_transactions", Number(event))
									}
									error={
										form.errors.recent_transactions &&
										useTranslateFormFields("recent_transactions.error")
									}
									rightSection={
										<TooltipIcon
											label={useTranslateFormFields(
												"recent_transactions.tooltip",
											)}
										/>
									}
									radius="md"
						...
						<Grid.Col span={8}>
							<SearchField
								...
								onCreate={() => setMode(Mode.CreateOrUpdate)}
							/>
							<DataTable
								height={"50vh"}
								withColumnBorders
									...
									},
									{
										...
										render: (record) => (
											<Group gap={"sm"} justify="flex-end">
												<ActionWithTooltip
													tooltip={useTranslateDataGridBaseColumns(
														"actions.buttons.edit.tooltip",
													)}
													icon={faEdit}
													actionProps={{ size: "sm" }}
													iconProps={{ size: "xs" }}
												...
												<ActionWithTooltip
													tooltip={useTranslateDataGridBaseColumns(
														"actions.buttons.delete.tooltip",
													)}
													color={"red.7"}
													icon={faTrashCan}
													actionProps={{ size: "sm" }}
													...
													onClick={async (e) => {
														e.stopPropagation();
														const categories = form.values.categories.filter(
															(cat) => cat.name !== record.name,
														);
														form.setFieldValue("categories", categories);
													}}
												/>
