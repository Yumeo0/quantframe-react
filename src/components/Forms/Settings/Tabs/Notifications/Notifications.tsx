...
export const NotificationsPanel = ({
	...
	const useTranslateForm = (
		...
		useTranslateForms(
			`settings.tabs.notifications.${key}`,
			{ ...context },
			i18Key,
		);
	...
	const isEnabled = (da: TauriTypes.NotificationSetting) => {
		return (
			da.system_notify.enabled ||
			da.discord_notify.enabled ||
			da.webhook_notify.enabled
		);
	};
	...
	}[] = [
		{
			...
			component: (
				<EditNotificationSetting
					...
					onChange={(newValue) =>
						form.setFieldValue("on_new_conversation", newValue)
					}
				/>
			),
			id: "on_new_conversation",
		...
		},
		{
			...
			component: (
				<EditNotificationSetting
					...
					onChange={(newValue) =>
						form.setFieldValue("on_wfm_chat_message", newValue)
					}
				/>
			),
			id: "on_wfm_chat_message",
					...
					<Tabs.List>
						{tabs.map((tab) => (
							<Tabs.Tab value={tab.id} key={tab.id}>
								<Text
									size="sm"
									c={isEnabled(form.values[tab.id]) ? "green.7" : "red.7"}
								>
									{tab.label}
								</Text>
							</Tabs.Tab>
