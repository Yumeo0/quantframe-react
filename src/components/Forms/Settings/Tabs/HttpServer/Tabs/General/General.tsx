...
export const GeneralPanel = ({ value, onSubmit }: GeneralPanelProps) => {
	...
	const useTranslateForm = (
		...
		useTranslateForms(
			`settings.tabs.http_server.general.${key}`,
			{ ...context },
			i18Key,
		);
				...
				<Stack>
					<Tooltip label={useTranslateFormFields("enabled.tooltip")}>
						<Checkbox
							...
							error={
								form.errors.enable && useTranslateFormFields("enable.error")
							}
						/>
					</Tooltip>
					<Collapse in={form.values.enable}>
						<TextInput
							...
							rightSection={
								<TooltipIcon label={useTranslateFormFields("host.tooltip")} />
							}
						/>
						<NumberInput
							...
							rightSection={
								<TooltipIcon label={useTranslateFormFields("port.tooltip")} />
							}
						/>
					</Collapse>
				</Stack>
