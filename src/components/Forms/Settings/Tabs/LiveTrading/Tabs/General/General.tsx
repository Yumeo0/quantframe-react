...
import { type FieldFilter, Operator, OperatorType } from "@utils/filter.helper";
...
export const GeneralPanel = ({
	...
	const useTranslateForm = (
		...
		useTranslateForms(
			`settings.tabs.live_scraper.general.${key}`,
			{ ...context },
			i18Key,
		);
		...
		setAvailableItems(
			tradableItems.map((item) => {
				return {
					...
					tags: item.tags,
					disabled_for:
						value.stock_item.blacklist?.find((bl) => bl.wfm_id === item.wfm_id)
							?.disabled_for || [],
				} as BlackList;
			}),
		);
	}, [tradableItems, value.stock_item.blacklist?.find]);
				...
				>
					<Group gap="md">
						<Select
							...
							description={useTranslateFormFields(
								`stock_mode.description.${form.values.stock_mode}`,
							)}
							...
							onChange={(event) =>
								form.setFieldValue("stock_mode", event as TauriTypes.StockMode)
							}
							error={
								form.errors.stock_mode &&
								useTranslateFormFields("stock_mode.error")
							}
							radius="md"
						/>
						<MultiSelect
							disabled={
								form.values.stock_mode !== TauriTypes.StockMode.Item &&
								form.values.stock_mode !== TauriTypes.StockMode.All
							}
							label={useTranslateFormFields("trade_modes.label")}
							w={250}
							description={useTranslateFormFields(`trade_modes.description`)}
							...
							onChange={(event) =>
								form.setFieldValue(
									"trade_modes",
									event as TauriTypes.TradeMode[],
								)
							}
							error={
								form.errors.trade_modes &&
								useTranslateFormFields("trade_mode.error")
							}
							radius="md"
						/>
					</Group>
					<Group gap={"md"} mt={25}>
						<Tooltip label={useTranslateFormFields("report_to_wfm.tooltip")}>
							<Checkbox
								...
								error={
									form.errors.report_to_wfm &&
									useTranslateFormFields("report_to_wfm.error")
								}
							/>
						</Tooltip>
						<Tooltip label={useTranslateFormFields("auto_delete.tooltip")}>
							<Checkbox
								...
								onChange={(event) =>
									form.setFieldValue("auto_delete", event.currentTarget.checked)
								}
								error={
									form.errors.auto_delete &&
									useTranslateFormFields("auto_delete.error")
								}
							/>
						</Tooltip>
						<Tooltip label={useTranslateFormFields("auto_trade.tooltip")}>
							<Checkbox
								...
								onChange={(event) =>
									form.setFieldValue("auto_trade", event.currentTarget.checked)
								}
								error={
									form.errors.auto_trade &&
									useTranslateFormFields("auto_trade.error")
								}
							/>
						</Tooltip>
						<Tooltip
							label={useTranslateFormFields(
								"should_delete_other_types.tooltip",
							)}
						>
							<Checkbox
								label={useTranslateFormFields(
									"should_delete_other_types.label",
								)}
								...
								error={
									form.errors.should_delete_other_types &&
									useTranslateFormFields("should_delete_other_types.error")
								}
							/>
						</Tooltip>
					</Group>
					...
					<Group gap="md">
						...
						>
							{useTranslateForm("buttons.edit_blacklist_label", {
								count: form.values.stock_item.blacklist?.length || 0,
							})}
						</Button>
						<Button
							onClick={() => {
						...
						>
							{useTranslateForm("buttons.edit_buy_list_label", {
								count: form.values.stock_item.buy_list?.length || 0,
							})}
						</Button>
					</Group>
					<Group
			...
			{viewMode === ViewMode.Blacklist && (
				<Stack>
					<SelectMultipleItems<BlackList>
						...
						onBeforeAdd={(item) => {
							if (
								!formLeft.values.disable_for ||
								formLeft.values.disable_for.length === 0
							) {
								notifications.show({
									color: "red.7",
									message: useTranslateBlacklist("no_trade_selected"),
								});
								open();
								return false;
							}
							...
							columns: [
								{
									sortable: true,
									accessor: "name",
									title: useTranslateBlacklist("name_title"),
								},
								{
									sortable: true,
									accessor: "trade_tax",
									title: useTranslateBlacklist("trade_tax_title"),
								},
								{
									sortable: true,
									accessor: "mr_requirement",
									title: useTranslateBlacklist("mr_requirement_title"),
								},
							],
							searchable: true,
								...
								<Paper p={"sm"} mt={"md"}>
									<Group>
										<Flex direction="column">
											<TextTranslate
												i18nKey={useTranslateBlacklist(
													"mr_requirement_label",
													undefined,
													true,
												)}
												values={{
													min: formLeft.values.mrRequirementRange?.at(0) || 0,
													max: formLeft.values.mrRequirementRange?.at(1) || 15,
												}}
											/>
											<RangeSlider
												w={"150px"}
										...
										<Flex direction="column">
											<TextTranslate
												...
												values={{
													min: formLeft.values.tradeTaxRange?.at(0) || 0,
													max: formLeft.values.tradeTaxRange?.at(1) || 2100000,
												}}
											/>
											<RangeSlider
												w={"150px"}
												...
												min={0}
												max={2100000}
												{...formLeft.getInputProps("tradeTaxRange")}
											/>
										</Flex>
									</Group>
									<Group>
										<SelectItemTags
											value={formLeft.values.tags || []}
											onChange={(value) =>
												formLeft.setFieldValue("tags", value)
											}
										/>
									</Group>
								</Paper>
								...
								<ActionWithTooltip
									...
									tooltip={useTranslateBlacklist("set_trade_modes_tooltip", {
										modes:
											formLeft.values.disable_for &&
											formLeft.values.disable_for.length > 0
												? formLeft.values.disable_for.join(", ")
												: "N/A",
									})}
									icon={faHandshake}
									actionProps={{ size: "sm" }}
						...
						rightConfig={{
							...
							columns: [
								...
								},
								{
									...
									render: (item) =>
										item.disabled_for
											?.map((mode) => useTranslateTradeMode(mode))
											.join(", ") || "N/A",
								},
							],
						}}
			...
			{viewMode === ViewMode.BuyList && (
				<Stack>
					<CreateItemForm
						...
						onSubmit={(values) => {
							if (
								form.values.stock_item.buy_list.find(
									(buyItem) => buyItem.wfm_id === values.raw,
								) ||
								values.bought <= 0
							)
								return;
							form.setFieldValue("stock_item", {
								...form.values.stock_item,
								buy_list: form.values.stock_item.buy_list.concat([
									{ wfm_id: values.raw, max_price: values.bought },
								]),
							});
						}}
					/>
					<DataTable
						...
						columns={[
							...
							},
							{
								...
								render: (item) => (
									<ActionWithTooltip
										tooltip={useTranslateCommon(
											"datatable_columns.actions.buttons.delete_tooltip",
										)}
										icon={faTrash}
										color="red.7"
										actionProps={{ size: "sm" }}
										...
										onClick={() => {
											form.setFieldValue("stock_item", {
												...form.values.stock_item,
												buy_list: form.values.stock_item.buy_list.filter(
													(buyItem) => buyItem.wfm_id !== item.wfm_id,
												),
											});
										}}
									/>
