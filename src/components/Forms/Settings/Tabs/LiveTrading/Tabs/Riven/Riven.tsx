...
export const RivenPanel = ({ value, onSubmit }: RivenPanelProps) => {
	...
	const useTranslateForm = (
		...
		useTranslateForms(
			`settings.tabs.live_scraper.riven.${key}`,
			{ ...context },
			i18Key,
		);
			...
			<form onSubmit={form.onSubmit((values) => onSubmit(values))}>
				<Group gap={"md"}>
					<NumberInput
						...
						onChange={(event) =>
							form.setFieldValue("min_profit", Number(event))
						}
						error={
							form.errors.min_profit &&
							useTranslateFormFields("min_profit.error")
						}
						rightSection={
							<TooltipIcon
								label={useTranslateFormFields("min_profit.tooltip")}
								link={useTranslateFormFields("min_profit.link")}
							/>
						}
						radius="md"
					/>
					<NumberInput
						...
						placeholder={useTranslateFormFields(
							"threshold_percentage.placeholder",
						)}
						value={form.values.threshold_percentage}
						onChange={(event) =>
							form.setFieldValue("threshold_percentage", Number(event))
						}
						error={
							form.errors.threshold_percentage &&
							useTranslateFormFields("threshold_percentage.error")
						}
						rightSection={
							<TooltipIcon
					...
					<NumberInput
						...
						error={
							form.errors.limit_to && useTranslateFormFields("limit_to.error")
						}
						rightSection={
							<TooltipIcon
								label={useTranslateFormFields("limit_to.tooltip")}
								link={useTranslateFormFields("limit_to.link")}
							/>
						}
						radius="md"
					/>
					<NumberInput
						...
						onChange={(event) =>
							form.setFieldValue("update_interval", Number(event))
						}
						error={
							form.errors.update_interval &&
							useTranslateFormFields("update_interval.error")
						}
						rightSection={
							<TooltipIcon
								label={useTranslateFormFields("update_interval.tooltip")}
								link={useTranslateFormFields("update_interval.link")}
							/>
						}
						radius="md"
					/>
