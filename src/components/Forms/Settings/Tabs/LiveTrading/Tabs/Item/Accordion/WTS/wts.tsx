...
export const WTSItemAccordion = ({
	value,
	onSubmit,
	onChange,
}: WTSItemAccordionProps) => {
	// Translate general
	const useTranslateForm = (
		...
		useTranslateForms(
			`settings.tabs.live_scraper.item.wts.${key}`,
			{ ...context },
			i18Key,
		);
			...
			<form onSubmit={form.onSubmit((values) => onSubmit(values))}>
				<Group gap={"md"}>
					...
					<NumberInput
						...
						error={
							form.errors.min_sma && useTranslateFormFields("min_sma.error")
						}
						rightSection={
							<TooltipIcon
								label={useTranslateFormFields("min_sma.tooltip")}
								link={useTranslateFormFields("min_sma.link")}
							/>
						}
						radius="md"
					/>
