...
export interface SelectCacheTradableItem
	extends Omit<TauriTypes.CacheTradableItem, "sub_type"> {
	label: string;
	value: string;
	available_sub_types?: TauriTypes.CacheTradableItemSubType;
	sub_type?: TauriTypes.SubType;
}
export function SelectTradableItem({
	hide_sub_type,
	value,
	onChange,
	description,
}: SelectTradableItemProps) {
	// State
	const [items, setItems] = useState<SelectCacheTradableItem[]>([]);
	const [filteredItems, setFilteredItems] = useState(items);
	const [lastKeyPressed, setLastKeyPressed] = useState<string | null>(null);
	const [selectedItem, setSelectedItem] =
		useState<SelectCacheTradableItem | null>(null);
	...
	const handleSelect = (item: SelectCacheTradableItem) => {
		...
		if (item.available_sub_types) {
			...
			if (sub_type.amber_stars || sub_type.cyan_stars)
				new_item.sub_type = {
					cyan_stars: sub_type.cyan_stars,
					amber_stars: sub_type.amber_stars,
				};
		}
		onChange(new_item);
	...
	return (
		<Group>
			...
			/>
			{selectedItem?.available_sub_types && !hide_sub_type && (
				<Group>
					{selectedItem.available_sub_types.variants && (
						<Select
							...
							description={
								description ? useTranslateFormFields("variant.description") : ""
							}
							data={selectedItem.available_sub_types.variants.map(
								(variant) => ({ label: upperFirst(variant), value: variant }),
							)}
							required
							value={
								selectedItem.sub_type?.variant ||
								selectedItem.available_sub_types.variants[0] ||
								""
							}
							onChange={(variant) => {
					...
					{selectedItem.available_sub_types.max_rank && (
						<NumberInput
							...
							description={
								description ? useTranslateFormFields("rank.description") : ""
							}
					...
					{selectedItem.available_sub_types.cyan_stars && (
						<NumberInput
							...
							description={
								description
									? useTranslateFormFields("cyan_stars.description")
									: ""
							}
							...
							onChange={(event) =>
								handleSubTypeUpdate({
									...
									amber_stars: selectedItem.available_sub_types?.amber_stars
										? selectedItem.sub_type?.amber_stars
										: undefined,
								})
							}
						/>
					...
					{selectedItem.available_sub_types.amber_stars && (
						<NumberInput
							...
							description={
								description
									? useTranslateFormFields("amber_stars.description")
									: ""
							}
							...
							onChange={(event) =>
								handleSubTypeUpdate({
									...
									cyan_stars: selectedItem.available_sub_types?.cyan_stars
										? selectedItem.sub_type?.cyan_stars
										: undefined,
								})
							}
						/>
