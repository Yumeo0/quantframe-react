...
export function CreateRivenAttributes({
	maxPositive,
	maxNegative,
	attributes,
	onSubmit,
}: CreateRivenAttributesProps) {
	// State
	...
	const addButtonText = useTranslateFormButtons("add");

	// Form
	const form = useForm({
		initialValues: {
			positive_attributes: Array.from(
				{ length: maxPositive },
				() => defaultAttribute as RivenAttribute,
			),
			negative_attributes: Array.from(
				{ length: maxNegative },
				() => ({ ...defaultAttribute, positive: false }) as RivenAttribute,
			),
		},
		onValuesChange: (values) => {
			const items = [
				...values.positive_attributes,
				...values.negative_attributes,
			];
			setCurrentAttribute(items);
			onSubmit?.(
				items.filter((item) => item.url_name !== "N/A" && item.url_name !== ""),
			);
		},
	});

	const GetAvailableAttributes = (
		currentAttribute: RivenAttribute | undefined,
	) => {
		...
		const avAttributes = attributes.filter(
			(item) => !formAttributes?.includes(item.url_name),
		);

		if (currentAttribute) {
			const attr = attributes.find(
				(item) => item.url_name === currentAttribute.url_name,
			);
	...
	return (
		<Stack w={"100%"} gap={"sm"}>
			...
			{form.values.positive_attributes
				.slice(0, showPositiveCount)
				.map((item, index) => {
					return (
						<CreateRivenAttribute
							key={`${item.positive}${item.effect}${item.value}`}
