...
import { Box, type BoxProps, Group, NumberInput } from "@mantine/core";
...
export function CreateItemForm({
	...
	const quantityErrorText = useTranslateFormFields("quantity.error");
	const boughtErrorText = useTranslateFormFields("bought.error");
		...
		<Box {...boxProps}>
			<form
				onSubmit={form.onSubmit((data) => {
					...
					onSubmit({
						...data,
						raw: idField ? (data as any)[idField] : data.wfm_url,
					});
				})}
			>
				<Group gap="md">
					...
					<NumberInput
						...
						error={form.errors.quantity ? quantityErrorText : undefined}
						radius="md"
						rightSection={
							<ActionWithTooltip
					...
					<NumberInput
						...
						error={form.errors.bought && boughtErrorText}
						radius="md"
						rightSection={
							<ActionWithTooltip
								tooltip={useTranslateButtons(
