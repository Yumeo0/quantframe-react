import {
	GenericItemList,
	type GenericItemListProps,
} from "@components/Forms/GenericItemList";
...
export function SelectMultipleItems<T>({
	...
	useEffect(() => {
		...
		setRightItems(right);
		setLeftItems(left);
	}, [selectedItems, leftConfig.items, getId]);
	...
	const OnAddAll = async (items: T[]) => {
		...
		setLeftItems(
			leftItems.filter((i) => !items.some((j) => getId(j) === getId(i))),
		);
	};
	...
	const OnRemoveAll = async (items: T[]) => {
		...
		updateRight(
			rightItems.filter((i) => !items.some((j) => getId(j) === getId(i))),
		);
