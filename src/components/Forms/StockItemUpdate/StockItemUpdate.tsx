		...
		<form onSubmit={form.onSubmit(async (values) => onUpdate?.(values))}>
			...
			<Box h={"50vh"}>
				...
				<NumberInput
					...
					rightSection={
						<TooltipIcon label={useTranslateFields("minimum_sma_tooltip")} />
					}
				/>
				<NumberInput
					min={-1}
					...
					onChange={(value) =>
						form.setFieldValue("minimum_profit", Number(value))
					}
					rightSection={
						<TooltipIcon label={useTranslateFields("minimum_profit_tooltip")} />
					}
				/>
				<NumberInput
					min={-1}
					...
					onChange={(value) =>
						form.setFieldValue("minimum_price", Number(value))
					}
					rightSection={
						<TooltipIcon label={useTranslateFields("minimum_price_tooltip")} />
					}
				/>
				<Group
					justify="flex-end"
