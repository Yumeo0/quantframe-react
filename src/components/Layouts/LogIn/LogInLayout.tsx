...
} from "@components/Layouts/Shared/NavbarMinimalColored";
...
export function LogInLayout() {
		...
		},
		{
			...
			icon: (
				<Indicator
					...
					label={
						(user?.unread_messages || 0) > 0 ? user?.unread_messages : undefined
					}
					inline
					size={16}
					position="top-start"
	...
	const handleAlertClick = (alert: QuantframeApiTypes.AlertDto) => {
		...
		const { event, payload } = alert.properties as {
			event: string;
			payload: any;
		};
		if (!event) return;
