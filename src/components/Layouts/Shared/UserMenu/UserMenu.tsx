...
export function UserMenu() {
	...
	// Translate general
	const useTranslateUserMenu = (
		key: string,
		context?: { [key: string]: any },
		i18Key?: boolean,
	) => useTranslateComponent(`user_menu.${key}`, { ...context }, i18Key);
	const useTranslateUserStatus = (
		key: string,
		context?: { [key: string]: any },
		i18Key?: boolean,
	...
	const logOutMutation = useMutation({
		...
		onSuccess: (u) => {
			notifications.show({
				title: useTranslateSuccess("logout.title"),
				message: useTranslateSuccess("logout.message"),
				color: "green.7",
			});
			SendTauriDataEvent(
				TauriTypes.Events.UpdateUser,
				TauriTypes.EventOperations.SET,
				u,
			);
		},
		onError: () =>
			notifications.show({
				title: useTranslateErrors("logout.title"),
				message: useTranslateErrors("logout.message"),
				color: "green.7",
			}),
	});
	...
	const IsConnected = () => {
		...
		if (app_error?.isWebSocketError()) return false;
				...
				<Group>
					...
					>
						<Avatar
							...
							variant="subtle"
							src={
								user?.wfm_avatar && user?.wfm_avatar !== ""
									? WFMThumbnail(user?.wfm_avatar)
									: "/default_avatar.png"
							}
