...
export function Clock() {
	...
	// Translate general
	const useTranslateClock = (
		key: string,
		context?: { [key: string]: any },
		i18Key?: boolean,
	) => useTranslateComponent(`clock.${key}`, { ...context }, i18Key);
	...
	return (
		<Group className={classes.clock}>
			<TextTranslate
				size="md"
				i18nKey={useTranslateClock("gmt", undefined, true)}
				values={{ time: currentTime }}
			/>
			<TextTranslate
				size="md"
				i18nKey={useTranslateClock("time_until_midnight", undefined, true)}
				values={{ time: timeUntilMidnight }}
			/>
		</Group>
	);
}
