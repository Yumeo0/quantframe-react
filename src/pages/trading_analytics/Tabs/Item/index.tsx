...
import { type QuantframeApiTypes, TauriTypes } from "$types";
...
export const ItemPanel = ({ isActive }: ItemPanelProps = {}) => {
	...
	const queryData = useForm({
		initialValues: {
			...
			limit: 10,
			query: "",
		} as QuantframeApiTypes.ItemPriceControllerGetListParams,
		validate: {
	...
	useEffect(() => {
		HasPermission(TauriTypes.PermissionsFlags.EXPORT_DATA).then((res) =>
			setCanExport(res),
		);
	}, []);
	// Queries
	const { paginationQuery, refetchQueries } = useQueries({
		queryData: queryData.values,
		isActive,
	});
	...
	}, [dates, queryData.setFieldValue]);

	const IsLoading = () =>
		(paginationQuery.isFetching || exportMutation.isPending) && isActive;
	...
	return (
		<Box p={"md"}>
				...
				rightSection={
					<Group gap={3}>
						<DatePickerInput
							...
							minDate={dayjs(queryData.values.to_date)
								.subtract(90, "day")
								.format("YYYY-MM-DD")}
							...
							<Grid.Col span={9}>
								<SimpleGrid cols={3} spacing={"sm"}>
									<MinMax
										...
										value={[
											queryData.values.volume_gt,
											queryData.values.volume_lt,
										]}
										onChange={(value) => {
											if (!value) return;
											queryData.setFieldValue("volume_gt", value[0]);
											queryData.setFieldValue(
												"volume_lt",
												value[1] || undefined,
											);
										}}
									...
									<MinMax
										...
										value={[
											queryData.values.supply_gt,
											queryData.values.supply_lt,
										]}
										onChange={(value) => {
											if (!value) return;
									...
									<MinMax
										...
										value={[
											queryData.values.demand_gt,
											queryData.values.demand_lt,
										]}
										onChange={(value) => {
											if (!value) return;
									...
									<MinMax
										...
										value={[
											queryData.values.min_price_gt,
											queryData.values.min_price_lt,
										]}
										onChange={(value) => {
											if (!value) return;
									...
									<MinMax
										...
										value={[
											queryData.values.max_price_gt,
											queryData.values.max_price_lt,
										]}
										onChange={(value) => {
											if (!value) return;
			...
			<DataTable
				...
				page={getSafePage(
					queryData.values.page,
					paginationQuery.data?.total_pages,
				)}
				...
				onRecordsPerPageChange={(limit) =>
					queryData.setFieldValue("limit", limit)
				}
				...
				columns={[
					...
					},
					{
						...
						render: ({ volume }) => (
							<NumberFormatter decimalScale={2} value={volume} />
						),
					},
					{
						accessor: "min_price",
						...
						render: ({ min_price }) => (
							<NumberFormatter decimalScale={2} value={min_price} />
						),
					},
					{
						accessor: "max_price",
						...
						render: ({ max_price }) => (
							<NumberFormatter decimalScale={2} value={max_price} />
						),
					},
					{
						accessor: "open_price",
						...
						render: ({ open_price }) => (
							<NumberFormatter decimalScale={2} value={open_price} />
						),
					},
					{
						...
						render: ({ closed_price }) => (
							<NumberFormatter decimalScale={2} value={closed_price} />
						),
					},
					{
						accessor: "avg_price",
						...
						render: ({ avg_price }) => (
							<NumberFormatter decimalScale={2} value={avg_price} />
						),
					},
					{
						accessor: "wa_price",
						...
						render: ({ wa_price }) => (
							<NumberFormatter decimalScale={2} value={wa_price} />
						),
					},
					{
						accessor: "median",
						...
						render: ({ median }) => (
							<NumberFormatter decimalScale={2} value={median} />
						),
					},
					{
						accessor: "moving_avg",
						...
						render: ({ moving_avg }) => (
							<NumberFormatter decimalScale={2} value={moving_avg} />
						),
					},
					{
						accessor: "donch_top",
						...
						render: ({ donch_top }) => (
							<NumberFormatter decimalScale={2} value={donch_top} />
						),
					},
					{
						accessor: "donch_bot",
						...
						render: ({ donch_bot }) => (
							<NumberFormatter decimalScale={2} value={donch_bot} />
						),
					},
					{
						...
						render: ({ trading_tax }) => (
							<NumberFormatter decimalScale={2} value={trading_tax} />
						),
					},
					{
						accessor: "supply",
						...
						render: ({ supply }) => (
							<NumberFormatter decimalScale={2} value={supply} />
						),
					},
					{
						accessor: "demand",
						...
						render: ({ demand }) => (
							<NumberFormatter decimalScale={2} value={demand} />
						),
					},
				]}
			/>
