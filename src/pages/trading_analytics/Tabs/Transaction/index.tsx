...
export const TransactionPanel = ({ isActive }: TransactionPanelProps = {}) => {
	...
	const [queryData, setQueryData] =
		useLocalStorage<TauriTypes.TransactionControllerGetListParams>({
	...
	const useTranslateTransactionType = (
		key: string,
		context?: { [key: string]: any },
		i18Key?: boolean,
	) => useTranslateEnums(`transaction_type.${key}`, { ...context }, i18Key);
	const useTranslateTransactionItemType = (
		key: string,
		context?: { [key: string]: any },
		i18Key?: boolean,
	...
	// Queries
	const { paginationQuery, financialReportQuery, refetchQueries } = useQueries({
		queryData,
		isActive,
	});
	const handleRefresh = () => {
	...
	// Mutations
	const { exportMutation, updateMutation, deleteMutation, deleteBulkMutation } =
		useMutations({
			refetchQueries,
			setLoadingRows,
			setSelectedRecords,
			...
			<Grid>
				<Grid.Col span={showReport ? 7 : 12}>
							...
							<Paper p={"sm"} mt={"md"}>
								<Group>
									<SelectItemTags
										...
										onChange={(value) =>
											setQueryData((prev) => ({ ...prev, tags: value }))
										}
									/>
									<DatePickerInput
										clearable
										...
										value={[
											queryData.from_date
												? new Date(queryData.from_date)
												: null,
											queryData.to_date ? new Date(queryData.to_date) : null,
										]}
										onChange={(value) => {
							...
							<Group gap={3}>
								...
								<ActionWithTooltip
									...
									actionProps={{
										size: "sm",
										disabled:
											selectedRecords.length === 0 || deleteMutation.isPending,
									}}
									onClick={() =>
										OpenDeleteBulkModal(
					...
					<Group gap={"md"} mt={"md"} grow>
						<Group>
							{Object.values([
								TauriTypes.TransactionType.Purchase,
								TauriTypes.TransactionType.Sale,
							]).map((status) => (
								<ColorInfo
									...
									onClick={() =>
										setQueryData((prev) => ({
											...prev,
											transaction_type:
												status === prev.transaction_type ? undefined : status,
										}))
									}
									infoProps={{
										"data-color-mode": "bg",
							...
							{Object.values(TauriTypes.TransactionItemType).map((type) => (
								<ColorInfo
									...
									onClick={() =>
										setQueryData((prev) => ({
											...prev,
											item_type: type === prev.item_type ? undefined : type,
										}))
									}
									infoProps={{
										"data-color-mode": "bg",
						...
						columns={[
							{
								...
								width: 250,
								render: (row) => (
									<ItemName color="gray.4" size="md" value={row} />
								),
							},
							{
								accessor: "item_type",
							...
							},
							{
								...
								render: ({ created_at }) => {
									return (
										<Text>{dayjs(created_at).format("DD.MM.YYYY HH:mm")}</Text>
									);
								},
							},
							{
									...
									<Group gap={3}>
										<ActionWithTooltip
											tooltip={useTranslateCommon(
												"datatable_columns.actions.buttons.edit_tooltip",
											)}
											icon={faHammer}
				...
				{showReport && (
					<Grid.Col span={5}>
						<FinancialReportCard
							data={financialReportQuery.data || null}
							loading={financialReportQuery.isLoading}
						/>
					</Grid.Col>
				)}
			</Grid>
