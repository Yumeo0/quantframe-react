...
interface MutationHooks {
	...
	setSelectedRecords?: (
		callback: (
			prev: TauriTypes.TransactionDto[],
...
const createGenericMutation = <TData, TVariables>(
	...
	return useMutation({
		...
		onError: (e: any) => {
			...
			notifications.show({
				title: useTranslateCommon(
					`notifications.${config.errorKey}.error.title`,
				),
				message: useTranslateCommon(
					`notifications.${config.errorKey}.error.message`,
				),
				color: "red.7",
			});
		},
...
export const useMutations = ({
	...
	const exportMutation = createGenericMutation(
		{
			mutationFn: (data: TauriTypes.TransactionControllerGetListParams) =>
				api.transaction.exportJson(data),
	...
	const updateMutation = createGenericMutation(
		{
			mutationFn: (data: TauriTypes.UpdateTransaction) =>
				api.transaction.update(data),
			successKey: "update_transaction",
			errorKey: "update_transaction",
			getLoadingId: (variables: TauriTypes.UpdateTransaction) =>
				`${variables.id}`,
