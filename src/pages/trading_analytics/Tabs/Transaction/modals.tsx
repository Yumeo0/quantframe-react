...
export const useModals = ({
	...
	const OpenDeleteBulkModal = (ids: number[]) => {
		modals.openConfirmModal({
			title: useTranslateCommon("prompts.delete_bulk_item.title", {
				count: ids.length,
			}),
			children: (
				<Text size="sm">
					{useTranslateCommon("prompts.delete_bulk_item.message", {
						count: ids.length,
					})}
				</Text>
			),
			labels: {
				confirm: useTranslateCommon("prompts.delete_bulk_item.confirm"),
				cancel: useTranslateCommon("prompts.delete_bulk_item.cancel"),
			},
			onConfirm: async () => {
