...
export const RivenPanel = ({ isActive }: RivenPanelProps = {}) => {
	...
	const queryData = useForm({
		initialValues: {
			...
			limit: 10,
			query: "",
		} as QuantframeApiTypes.RivenPriceControllerGetListParams,
		validate: {
	...
	useEffect(() => {
		...
		HasPermission(TauriTypes.PermissionsFlags.RIVEN_PRICES_SEARCH).then((res) =>
			setCanUse(res),
		);
	}, []);
	...
	const IsLoading = () =>
		(paginationQuery.isFetching || exportMutation.isPending) &&
		isActive &&
		canUse;
	...
	return (
		<Box p={"md"} pos={"relative"}>
			<PatreonOverlay
				permission={TauriTypes.PermissionsFlags.RIVEN_PRICES_SEARCH}
				tier="T2+"
			/>
			<SearchField
					...
					<Paper p={"sm"} mt={"md"}>
						<SimpleGrid cols={3} spacing={"sm"}>
							...
							<MinMax
								...
								value={[
									queryData.values.avg_price_gt,
									queryData.values.avg_price_lt,
								]}
								onChange={(value) => {
									if (!value) return;
			...
			<DataTable
				...
				columns={[
					...
					},
					{
						...
						render: ({ median_price }) => (
							<NumberFormatter decimalScale={2} value={median_price} />
						),
					},
				]}
			/>
