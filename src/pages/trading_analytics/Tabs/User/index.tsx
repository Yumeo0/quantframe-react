...
export const UserPanel = ({ isActive }: UserPanelProps = {}) => {
	...
	const queryData = useForm({
		initialValues: {
			group_by: "day",
		} as QuantframeApiTypes.WfmControllerGetUserActiveHistoryParams,
		validate: {
	...
	// Queries
	const { paginationQuery } = useQueries({
	...
	return (
		<Box p={"md"} pos={"relative"}>
			<PatreonOverlay
				permission={TauriTypes.PermissionsFlags.WFM_USER_ACTIVE_HISTORY}
				tier="T1+"
			/>
			<Grid>
						...
						<Group>
							<Select
								...
								data={[
									{
										value: "day",
										label: useTranslateFields("group_by.options.day"),
									},
									{
										value: "hour",
										label: useTranslateFields("group_by.options.hour"),
									},
								]}
								value={queryData.values.group_by}
								onChange={(value) =>
									queryData.setFieldValue("group_by", value as any)
								}
