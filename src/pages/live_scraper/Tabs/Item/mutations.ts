...
export const useMutations = ({
	...
	const exportMutation = createGenericMutation(
		{
			mutationFn: (data: TauriTypes.StockItemControllerGetListParams) =>
				api.stock_item.exportJson(data),
	...
	const createMutation = createGenericMutation(
		{
			mutationFn: (data: TauriTypes.CreateStockItem) =>
				api.stock_item.create(data),
	...
	const updateMutation = createGenericMutation(
		{
			mutationFn: (data: TauriTypes.UpdateStockItem) =>
				api.stock_item.update(data),
			successKey: "update_stock_item",
			errorKey: "update_stock_item",
			getLoadingId: (variables: TauriTypes.UpdateStockItem) =>
				`${variables.id}`,
			getSuccessMessage: (data: any) => ({ name: data.item_name }),
		},
		hooks,
	...
	const updateMultipleMutation = createGenericMutation(
		{
			...
			}) => api.stock_item.updateMultiple(data.ids, data.input),
			successKey: "update_stock_item",
			errorKey: "update_stock_item",
