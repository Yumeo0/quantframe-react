...
export const useStockModals = ({
		...
		modals.openContextModal({
			...
			innerProps: {
				fields: [
					{
						name: "maximum_price",
						label: useTranslateCommon(
							"prompts.maximum_price.fields.maximum_price.label",
						),
						attributes: {
							min: 0,
							description: useTranslateCommon(
								"prompts.maximum_price.fields.maximum_price.description",
							),
						},
						value: maximum_price,
						type: "number",
	...
	const OpenDeleteModal = (id: number) => {
		modals.openConfirmModal({
			...
			children: (
				<Text size="sm">
					{useTranslateCommon("prompts.delete_item.message", { count: 1 })}
				</Text>
			),
			labels: {
				confirm: useTranslateCommon("prompts.delete_item.confirm"),
				cancel: useTranslateCommon("prompts.delete_item.cancel"),
			},
	...
	const OpenDeleteMultipleModal = (ids: number[]) => {
		modals.openConfirmModal({
			...
			children: (
				<Text size="sm">
					{useTranslateCommon("prompts.delete_multiple_items.message", {
						count: ids.length,
					})}
				</Text>
			),
		...
		modals.openContextModal({
			...
			innerProps: {
				fields: [
					{
						name: "bought",
						label: useTranslateCommon(
							"prompts.bought_manual.fields.bought.label",
						),
						attributes: {
							min: 0,
						},
				...
				onConfirm: async (data: { bought: number }) => {
					if (!stock) return;
					const { bought } = data;
					await boughtMutation.mutateAsync({
						id: stock.id,
