...
export const WishListPanel = ({ isActive }: WishListPanelProps = {}) => {
	...
	const [queryData, setQueryData] =
		useLocalStorage<TauriTypes.WishListControllerGetListParams>({
	...
	const {
		...
		statusCountsQuery,
		refetchQueries,
	} = useWishListQueries({ queryData, isActive });
	...
	useTauriEvent(TauriTypes.Events.RefreshWishListItems, handleRefresh, [
		refetchQueries,
	]);
	return (
		<Box>
				...
				columns={[
					{
						...
						sortable: true,
						render: (row) => (
							<ItemName hideQuantity color="gray.4" size="md" value={row} />
						),
					},
					{
						accessor: "quantity",
					...
					},
					{
						...
						render: (row) => (
							<ColumnActions
								...
								onAuto={(price) =>
									boughtMutation.mutateAsync({ ...row, price })
								}
