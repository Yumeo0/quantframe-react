...
export const RivenPanel = ({ isActive }: RivenPanelProps = {}) => {
	...
	const [queryData, setQueryData] =
		useLocalStorage<TauriTypes.StockItemControllerGetListParams>({
	...
	const {
		...
		statusCountsQuery,
		refetchQueries,
	} = useStockQueries({ queryData, isActive });
	...
	useTauriEvent(TauriTypes.Events.RefreshStockRivens, handleRefresh, [
		refetchQueries,
	]);
	return (
		<Box>
			<Grid>
								...
								<ColorInfo
									...
									onClick={() =>
										setQueryData((prev) => ({
											...prev,
											status:
												(key as TauriTypes.StockStatus) === prev.status
													? undefined
													: (key as TauriTypes.StockStatus),
										}))
									}
									infoProps={{
										"data-color-mode": "bg",
				...
				<Grid.Col span={4}>
					<StatsWithSegments
						...
						segments={[
							{
								label: useTranslateSegments("bought"),
								count: financialReportQuery.data?.expenses || 0,
								color: "var(--qf-negative-color)",
							},
							{
								label: useTranslateSegments("listed"),
								count: financialReportQuery.data?.revenue || 0,
								color: "var(--qf-positive-color)",
							},
							{
								label: useTranslateSegments("profit"),
				...
				rightSection={
					<Group gap={3}>
						...
						<ActionWithTooltip
							...
							onClick={() =>
								OpenUpdateMultipleModal(selectedRecords.map((r) => r.id))
							}
						/>
						<ActionWithTooltip
							...
							onClick={() =>
								OpenDeleteMultipleModal(selectedRecords.map((r) => r.id))
							}
						/>
					</Group>
				}
			...
			<DataTable
				...
				onSortStatusChange={(sort) => {
					...
					setQueryData((prev) => ({
						...prev,
						sort_by: sort.columnAccessor as string,
				...
				onCellClick={({ record, column }) => {
					switch (column.accessor) {
						case "weapon_name": {
							...
							notifications.show({
								title: useTranslateCommon(
									"notifications.copy_to_clipboard.title",
								),
				...
				columns={[
					...
					},
					{
						...
						render: ({ attributes }) => (
							<RivenAttributes tooltip={!isWide} attributes={attributes} />
						),
					},
					{
						accessor: "bought",
					...
					},
					{
						...
						render: ({ id, minimum_price }) => (
							<ColumnMinMaxPrice
								...
								onUpdate={async (id: number, minimum_price: number) =>
									await updateMutation.mutateAsync({ id, minimum_price })
								}
								onEdit={async (id: number, minimum_price: number) =>
									OpenMinimumPriceModal(id, minimum_price)
								}
							/>
						),
					},
						...
						render: (row) => (
							<ColumnActions
								...
								onManual={() =>
									OpenSellModal({
										...row,
										wfm_url: row.wfm_weapon_url,
										rank: row.sub_type?.rank || 0,
										price: 0,
									})
								}
								onAuto={(price) =>
									sellMutation.mutateAsync({
										...row,
										wfm_url: row.wfm_weapon_url,
										rank: row.sub_type?.rank || 0,
										price,
								...
								onHide={(hide) =>
									updateMutation.mutateAsync({ id: row.id, is_hidden: hide })
								}
