...
import {
	...
	type MutationHooks,
} from "@utils/genericMutation.helper";
...
export const useStockMutations = ({
	...
	const exportMutation = createGenericMutation(
		{
			mutationFn: (data: TauriTypes.StockItemControllerGetListParams) =>
				api.stock_riven.exportJson(data),
	...
	const createMutation = createGenericMutation(
		{
			mutationFn: (data: TauriTypes.CreateStockRiven) =>
				api.stock_riven.create(data),
			successKey: "create_stock_riven",
			errorKey: "create_stock_riven",
			getSuccessMessage: (data: any) => ({
				name: `${data.weapon_name} ${data.mod_name}`,
			}),
		},
		hooks,
	);
	...
	const updateMutation = createGenericMutation(
		{
			mutationFn: (data: TauriTypes.UpdateStockRiven) =>
				api.stock_riven.update(data),
			successKey: "update_stock_riven",
			errorKey: "update_stock_riven",
			getLoadingId: (variables: TauriTypes.UpdateStockRiven) =>
				`${variables.id}`,
			getSuccessMessage: (data: any) => ({
				name: `${data.weapon_name} ${data.mod_name}`,
			}),
	...
	const updateMultipleMutation = createGenericMutation(
		{
			...
			}) => api.stock_riven.updateMultiple(data.ids, data.input),
			successKey: "update_stock_riven",
			errorKey: "update_stock_riven",
			isMultiple: (variables: {
				ids: number[];
				input: TauriTypes.UpdateStockItem;
			}) => variables.ids.length > 1,
			getLoadingId: (variables: {
				ids: number[];
				input: TauriTypes.UpdateStockItem;
	...
	const sellMutation = createGenericMutation(
		{
			mutationFn: (data: TauriTypes.SellStockRiven) =>
				api.stock_riven.sell(data),
