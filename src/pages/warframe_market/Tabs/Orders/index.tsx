...
export const OrderPanel = ({ isActive }: OrderPanelProps) => {
	const [queryData, setQueryData] =
		useState<WFMarketTypes.WfmOrderControllerGetListParams>({
			page: 1,
			limit: 25,
			sort_by: "order_type",
			sort_direction: "desc",
	...
	// Translate general
	const useTranslateTabOrder = (
		key: string,
		context?: { [key: string]: any },
		i18Key?: boolean,
	) =>
		useTranslatePages(
			`warframe_market.tabs.orders.${key}`,
			{ ...context },
			i18Key,
		);
	...
	const useTranslateErrors = (
		key: string,
		context?: { [key: string]: any },
		i18Key?: boolean,
	) => useTranslateTabOrder(`errors.${key}`, { ...context }, i18Key);
	const useTranslateSuccess = (
		key: string,
		context?: { [key: string]: any },
		i18Key?: boolean,
	...
	// Queries
	const { refetchQueries, paginationQuery, statusCountsQuery } =
	...
	useTauriEvent(TauriTypes.Events.RefreshWfmOrders, handleRefresh, [
		refetchQueries,
	]);
		...
		<Box>
							...
							<ColorInfo
								...
								onClick={() =>
									setQueryData((prev) => ({
										...prev,
										order_type:
											(key as WFMarketTypes.OrderType) === prev.order_type
												? undefined
												: (key as WFMarketTypes.OrderType),
									}))
								}
								infoProps={{
									"data-color-mode": "bg",
				...
				<Group gap="xs">
					<Select
						...
						onChange={(value) =>
							setQueryData((prev) => ({
								...prev,
								sort_by: value || "platinum",
							}))
						}
						data={["created_at", "platinum", "updated_at", "order_type"].map(
							(key) => ({
								label: useTranslateCommon(`sort_by.${key}`),
								value: key,
							}),
						)}
						size="xs"
					...
					<ActionIcon
						...
						onClick={() => {
							const direction =
								queryData.sort_direction === "asc" ? "desc" : "asc";
			...
			>
				{deleteAllOrdersMutation.isPending && (
					<Loading
						text={`${deletingOrders.current} / ${deletingOrders.total}`}
					/>
				)}
								...
								<>
									<ActionWithTooltip
										tooltip={useTranslateButtons(
											"sell_manual." +
												(order.type === WFMarketTypes.OrderType.Buy
													? "buy_tooltip"
													: "sell_tooltip"),
										)}
										icon={faPen}
			...
			<Group grow mt={"md"}>
				<Group justify="flex-end">
					<Pagination
						...
						total={Math.ceil(
							(paginationQuery.data?.total || 0) / queryData.limit,
						)}
					/>
				</Group>
			</Group>
