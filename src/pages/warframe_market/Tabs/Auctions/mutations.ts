		...
		onSuccess: (data: TData, variables: TVariables) => {
			...
			notifications.show({
				...
				message: config.translateCommon
					...
					: hooks.useTranslateSuccess(
							`${config.successKey}.message`,
							config.getSuccessMessage
								? config.getSuccessMessage(data, variables)
								: {},
						),
		...
		onError: (e: any) => {
			...
			notifications.show({
				...
				message: config.translateCommon
					...
					: hooks.useTranslateErrors(`${config.errorKey}.message`, {
							error: e,
						}),
				color: "red.7",
			});
...
export const useStockMutations = ({
	...
	const importStockMutation = createGenericMutation(
		{
			mutationFn: (data: { id: string; bought: number }) =>
				api.auction.importById(data.id, data.bought),
			successKey: "create_stock_riven",
			errorKey: "create_stock_riven",
			translateCommon: true,
			getLoadingId: (variables: { id: string; bought: number }) =>
				`${variables.id}`,
