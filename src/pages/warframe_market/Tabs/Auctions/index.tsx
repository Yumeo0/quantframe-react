...
import { TauriTypes, type WFMarketTypes } from "$types";
...
export const AuctionPanel = ({ isActive }: AuctionPanelProps) => {
	const [queryData, setQueryData] =
		useState<WFMarketTypes.WfmAuctionControllerGetListParams>({
			page: 1,
			limit: 12,
			sort_by: "created_at",
			sort_direction: "desc",
	...
	const useTranslateTabOrder = (
		...
		useTranslatePages(
			`warframe_market.tabs.auctions.${key}`,
			{ ...context },
			i18Key,
		);
	...
	// Queries
	const { refetchQueries, paginationQuery } = useStockQueries({
		queryData,
		isActive,
	});
	...
	useTauriEvent(TauriTypes.Events.RefreshWfmAuctions, handleRefresh, [
		refetchQueries,
	]);
		...
		<Box>
			...
			<Group gap={"sm"} mt={"md"} justify="flex-end">
				<Select
					...
					allowDeselect={false}
					onChange={(value) =>
						setQueryData((prev) => ({ ...prev, sort_by: value || "platinum" }))
					}
					data={["created_at", "platinum", "updated_at"].map((key) => ({
			...
			>
				{deleteAllAuctionsMutation.isPending && (
					<Loading
