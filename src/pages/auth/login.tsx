...
import { type ResponseError, TauriTypes } from "$types";
...
export default function LoginPage() {
	...
	const [banned_reason, setBannedReason] = useState<string | undefined>(
		undefined,
	);
		...
		onSuccess: async (u) => {
			...
			if (u.qf_banned)
				return notifications.show({
					...
					message: useTranslateErrors("login.banned"),
					color: "red.7",
				});
			if (!u.verification)
				return notifications.show({
					title: useTranslateErrors("login.title"),
					message: useTranslateErrors("login.verification"),
					color: "red.7",
				});
	...
	return (
		<Center w={"100%"} h={"92vh"}>
			<LogInForm
				...
				onSubmit={(d: { email: string; password: string }) =>
				...
				footerContent={
					<>
						...
						{is_banned && (
							<TextTranslate
								i18nKey={useTranslateErrors("login.ban_reason")}
