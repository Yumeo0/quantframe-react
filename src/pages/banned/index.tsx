	...
	return (
		<Center w={"100%"} h={"92vh"}>
			{user?.wfm_banned && (
				<Alert
					color="red"
					title={useTranslatePage("wfm.title")}
					icon={<FontAwesomeIcon icon={faExclamationTriangle} />}
					maw={"75%"}
				>
					<Text>
						{useTranslatePage("wfm.reason", {
							reason: user?.wfm_banned_reason,
						})}
					</Text>
					{endData && (
						<Text>
							{useTranslatePage("wfm.until", {
								reason: user?.qf_banned_reason,
							})}{" "}
							{endData && (
								<Countdown startDate={new Date()} endDate={endData} />
							)}
						</Text>
					)}
				</Alert>
			...
			{user?.qf_banned && (
				<Alert
					color="red"
					title={useTranslatePage("qf.title")}
					icon={<FontAwesomeIcon icon={faExclamationTriangle} />}
					maw={"75%"}
				>
					<Text>
						{useTranslatePage("qf.reason", { reason: user?.qf_banned_reason })}
					</Text>
					{endData && (
						<Text>
							{useTranslatePage("qf.until", { reason: user?.qf_banned_reason })}{" "}
							{endData && (
								<Countdown startDate={new Date()} endDate={endData} />
							)}
						</Text>
