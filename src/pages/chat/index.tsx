...
export default function ChatPage() {
	...
	const [queryData, setQueryData] =
		useLocalStorage<WFMarketTypes.WfmChatDataControllerGetListParams>({
			key: "chat_query_key",
	...
	const [activeChat, setActiveChat] = useState<
		WFMarketTypes.ChatData | undefined
	>(undefined);
	// Queries
	const { paginationQuery, refetchQueries, updateChatData } = useChatQueries({
		queryData,
	});
	// Queries
			...
			<Grid data-has-alert={useHasAlert()} className={`${classes.container}`}>
				<Grid.Col span={activeChat ? 4 : 12}>
					<SearchField
						...
						onChange={(text) =>
							setQueryData((prev) => ({ ...prev, query: text }))
						}
