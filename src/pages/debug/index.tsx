...
export default function DebugPage() {
	...
	const useTranslate = (
		...
		i18Key?: boolean,
	) => useTranslatePages(`debug.${key}`, { ...context }, i18Key);
	...
	const tabs = [
		{
			label: useTranslateTabs("logging.title"),
			component: <LoggingPanel />,
			id: "tr",
			icon: <div>Stocks</div>,
		},
		{
			label: useTranslateTabs("states.title"),
			component: <StatesPanel />,
			id: "states",
			icon: <div>States</div>,
		},
		{
			label: useTranslateTabs("debugging.title"),
			component: <DebuggingPanel />,
			id: "debugging",
			icon: <div>Debugging</div>,
		},
	];
	return (
		<Container
